...
{
    std::vector<Vector> normals(pvertices.size(), Vector(0, 0, 0));
    for (unsigned int i = 0; i < pfaces.size(); i++) {
        const Face& f = pfaces[i];
        for (int j = 0; j < f.numVertices(); j++) {
            normals[f.vertexIndex(j)] += f.normal();
        }
    }
...
void Object::computeVertexNormalsPerFace()
{
 ...
 void Object::setPosition(const Point &p) {
     Point c = pboundingBox.center();
     for (unsigned int i = 0; i < pvertices.size(); i++) {
         Vertex& v = pvertices[i];
         v.setCoord(v.coord() - c + p);
     }
 }

 Object Object::getCopy() const {
     Object res(this->pname);
     res.pfaces = this->pfaces;
     res.pvertices = this->pvertices;
 ...
 Object Object::toTriangles() const {

     vector<Face> newfaces;
     newfaces.reserve(pfaces.size());

     for (unsigned int i = 0; i < pfaces.size(); i++) {
         const Face& f = pfaces[i];
         for (int j = 2; j < f.numVertices(); j++) {
             Face nf(f.vertexIndex(0), f.vertexIndex(j-1), f.vertexIndex(j));
             nf.computeNormal(this->pvertices);
...
void read_mtllib( char **words, int nwords, const string& filename )
{
    ...
    if (!fp)
    {
        ...
        while (true) 
        {
            ...
            // added 
                ...
                read_mtllib (&words[1], nwords-1, filename);
            }
